...
const GRAMatrixPage = () => {
  ...
  useEffect(() => {
    ...
    if (userId) fetchMatrix();
  }, [userId]);

  useEffect(() => {
    async function checkMatrixTypes() {
      try {
  ...
  const handleConsiderMatrix = async (cooperation) => {
    try {
      ...
      if (res.data.matrix) {
        ...
        alert("Too many constraints to produce results. Consider reducing role count or constraints.");
      }
    } catch (err) {
